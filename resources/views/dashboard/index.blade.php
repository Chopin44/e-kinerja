<x-app-layout>
    <div class="space-y-6">
            ...
            <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $totalSubKegiatan ?? 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-cyan-500 to-cyan-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $totalRincian ?? 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ number_format($avgProgressFisik ?? 0, 1) }}%</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ number_format($avgProgressAnggaran ?? 0, 1) }}%</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $kegiatanOnTrack ?? 0 }}</p>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="text-center mb-6">
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Dari total pagu <b>Rp {{ number_format($totalPagu ?? 0, 0, ',', '.') }}</b>,
                    sudah terealisasi <b>Rp {{ number_format($totalRealisasi ?? 0, 0, ',', '.') }}</b>
                    ({{ number_format($persentaseRealisasi ?? 0, 1) }}%)
                </p>
            </div>

            <div class="flex justify-center items-center">
                <div class="relative w-44 h-44 sm:w-56 sm:h-56 md:w-64 md:h-64">
                    <canvas id="anggaranPieChart" data-total-pagu="{{ $totalPagu ?? 0 }}"
                        data-total-realisasi="{{ $totalRealisasi ?? 0 }}" class="w-full h-full"></canvas>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="p-6">
                @if(empty($bidangs) || count($bidangs) == 0)
                <p class="text-center text-gray-500 italic">Belum ada data bidang untuk tahun ini.</p>
                @else
                        ...
                        <div class="grid grid-cols-2 gap-4 text-center">
                            ...
                            <div>
                                <div class="text-2xl font-bold text-green-600">
                                    {{ number_format($bidang['avg_progress'] ?? 0, 0) }}%
                                </div>
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            @if(empty($kegiatanTerbaru) || $kegiatanTerbaru->isEmpty())
            <div class="p-10 text-center text-gray-500">
                ...
                <p class="text-sm text-gray-500 mt-1">Silakan tambahkan kegiatan baru di menu <b>Kegiatan</b>.</p>
            </div>
            @else
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm text-gray-500">
                                    Dibuat: {{ optional($kegiatan->created_at)->translatedFormat('d F Y') }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full"
                                        style="width: {{ $kegiatan->current_progress ?? 0 }}%"></div>
                                </div>
                                <div class="text-sm text-gray-500 mt-1">{{ number_format($kegiatan->current_progress ??
                                    0, 1) }}%</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="w-full bg-gray-200 rounded-full h-2">
